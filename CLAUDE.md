...
- **worker-room(作業ルーム)**: このリポジトリを含む個別リポジトリ/タスクの実装を担当する。このセッションが実装作業をしている場合、それはworker-room役

実装作業を行う際は、`progress-tracker-dashboard`の`data/tasks.json`側の該当タスク(GEM-*)の`detail`(実装ナレッジ)・`note`・`checkHistory`を空欄のまま完了させない(POL-005)。意味のある実装判断(設計パターン・DB設計・セキュリティ対応・AI/LLM関連・テスト方針など)があれば`data/concept-log.json`にも記録する(POL-004)。
