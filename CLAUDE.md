...
14. 回答・アウトプット・相談ごと・処理には、視認性のため通し番号やマーカーを付ける(2026-08-21追加)。
    1. 質問への回答やアウトプットを出す場合は、【回答MM/DD HH:MM:SS】(その時点の日時、例: 【回答09/25 21:47:33】)を付けて示す(2026-09-25変更、ユーザー指示「〇〇の通し番号は廃止にしろ、代わりに何月何日何時何分何秒に変えろ」)。以前は3桁の通し番号だったが、長時間・複数日にまたがるセッションでは通し番号を正確に管理し続けられない懸念があったため、常に一意に定まる日時形式へ変更した。ユーザーが返信する際も、この日時を使って回答してよい。
    2. マージしてよいか等、判断を仰ぐ相談ごとを出す場合は、【相談MM/DD HH:MM:SS】(その時点の日時、例: 【相談09/26 14:03:10】)を付けて示す(2026-09-26変更、1.と同じ理由・同じユーザー指示によりルーム内の3桁通し番号方式を廃止し日時形式へ統一)。
    3. 実装・調査など時間のかかる処理を行う場合は、着手時に【処理開始MM/DD HH:MM:SS】(着手時点の日時)を示し、完了時には同じ日時をそのまま再掲した【処理完了MM/DD HH:MM:SS】を示して、今どの段階かを分かりやすくする(通し番号と違い、着手時の日時そのものが開始・完了を結びつける識別子になるため、完了時に新しい日時を振り直さない。2026-09-26変更、同上の理由・指示)。
    4. 過去に自分から出した【相談MM/DD HH:MM:SS】のうち、ユーザーからまだ回答・解決されていないものがある場合は、その回のやり取りで明示的に「【相談MM/DD HH:MM:SS】は未解決です」のように報告する(放置して忘れさせない。2026-09-26、表記を日時形式に統一)。
    5. 「ルームごとの通し番号を管理する」という概念自体が2026-09-26に廃止された(1.〜4.参照)。日時は各ルーム・各AIがその場で独立に生成でき、複数ルームやセッション再開・コンテキスト圧縮をまたいでも共有カウンタを同期させ続ける必要がないため、番号の食い違い・重複・「今どこまで進んだか分からない」という問題自体が起きない。
15. 改修時は、その場しのぎの案は絶対に出さないこと。全ての構成データを見直し、エラーが出なく、尚且つその後の更新も考えた改修案を考案すること。
16. 定期的にCLAUDE.mdファイルの中身を確認すること。(2026-09-13追加、ユーザー指示)具体的な仕組みとして、progress-tracker-dashboardのセッションに対しRoutine「CLAUDE.md定期確認(平日0時/休日祝日12時間毎)」を設定済み。平日(祝日を除く)は日本時間0時に1回、土日祝日は日本時間0時・12時の12時間おきに発火し、CLAUDE.md(正本はdata/claude-core-rules.md)を読み直して見落とし・矛盾が無いか確認する。
...
23. 何かを提案する前に、必ず現状のデータ・構造・経緯(過去のコミット履歴・既存の仕様書・関連ドキュメント等)を確認し、理解した上で提案すること。新しい仕組みを次々に提案するのではなく、まず既存のシステムを改修することで実現できないかを検討し、それでも実現できない場合に限って新しい案を提案すること(2026-09-12追加、ユーザー指摘。study-appの履歴・メモの同期方式について、既存のGAS連携が廃止された経緯を確認しないまま新しい仕組みを提案したことがきっかけ)。
